...
export function mountStare(canvas: HTMLCanvasElement, opts: StareOpts): () => void {
  ...
  let ringTex: WebGLTexture | null = null;
  let faceTex: WebGLTexture | null = null;
  let wellTex: WebGLTexture | null = null;
  ...
  const resize = () => {
    ...
    view = { w: canvas.clientWidth || window.innerWidth, h: canvas.clientHeight || window.innerHeight, dpr };
...
function coverOf(iw: number, ih: number, vw: number, vh: number): [number, number] {
  const ir = iw / ih;
  const vr = vw / vh;
...
function makeTex(gl: WebGLRenderingContext, img: HTMLImageElement): WebGLTexture {
